"""
@file		nnstreamer_example_face_detection_rtmp.py
@date		19 Oct 2020
@brief		rtmp streaming version of face detection tflite
@see		https://github.com/nnstreamer/nnstreamer
@author		Jongha Jang <jangjongha.sw@gmail.com>
@bug		No known bugs.

rtmp streaming version of rtmp streaming version of face detection tflite.

Pipeline :
v4l2src -- tee -- videoconvert -- cairooverlay -- ximagesink
            |
            --- videoscale -- tensor_converter -- tensor_transform -- tensor_filter -- tensor_sink

Get model by
$ cd $NNST_ROOT/bin
$ bash get-model.sh face-detection-tflite

This code is a rtmp streaming version of face detection tflite. youtube auth rtmp key is required.

Run example :
Before running this example, GST_PLUGIN_PATH should be updated for nnstreamer plugin.
$ export GST_PLUGIN_PATH=$GST_PLUGIN_PATH:<nnstreamer plugin path>
$ python3 nnstreamer_example_face_detection_rtmp.py <youtube rtmp auth key>
...
class NNStreamerExample:
    """NNStreamer example for Face detection with rtmp support (Youtube)"""
    ...
    def run_example(self):
        ...
        print("Run: NNStreamer example Face Detection with rtmp support (Youtube)")
        ...
        self.pipeline = Gst.parse_launch(
            'v4l2src name=cam_src ! videoconvert ! videoscale ! video/x-raw,width=' + str(self.VIDEO_WIDTH) + ',height=' + str(self.VIDEO_HEIGHT) + ',format=RGB ! tee name=t_raw '
            't_raw. ! queue leaky=2 max-size-buffers=2 ! videoscale ! video/x-raw,width=' + str(self.MODEL_WIDTH) + ',height=' + str(self.MODEL_HEIGHT) + ' ! '
            'tensor_converter ! tensor_transform mode=arithmetic option=typecast:float32,add:-127.5,div:127.5 ! '
            'tensor_filter framework=tensorflow-lite model=' + self.tflite_model + ' ! tensor_sink name=res_face '
            't_raw. ! queue ! videoconvert ! cairooverlay name=tensor_res ! tee name=output_handler '
        ...
        self.set_window_title('output_local', 'NNStreamer example Face Detection with rtmp support (Youtube)')
