...
typedef struct _CustomData
{
  ...
  ArgData arg;                 /**< Arguments for pipeline */
} CustomData;
...
run_pipeline (void *userdata)
{
  ...
  if (!info->launch_pipeline (&data->pipeline, data->pipeline_option, &data->arg)) {
    message =
 ...
 */
static void
gst_native_init (JNIEnv * env, jobject thiz, jint media_w, jint media_h, jstring media_path)
{
  ...
  data->arg.media_width = (gint) media_w;
  data->arg.media_height = (gint) media_h;
  data->arg.media_path = (*env)->GetStringUTFChars(env, media_path, NULL);

  /* register nnstreamer plugins */
  GST_PLUGIN_STATIC_REGISTER (nnstreamer);

  /* register nnstreamer amcsrc */
...
static JNINativeMethod native_methods[] = {
  {"nativeInit", "(IILjava/lang/String;)V", (void *) gst_native_init},
